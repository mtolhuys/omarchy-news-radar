...
- Remote feed content, marketplace metadata, release notes, repository text, and community submissions are untrusted data. Render textual fields as bounded plain text and never execute them. Images are optional allowlisted marketplace preview HTTPS URLs (`image.sourceUrl` on `plugins.omarchy.org`); the publisher validates PNG/JPEG/WebP bounds at build time without hosting rasters, and clients must never render remote HTML, Markdown, SVG, or arbitrary image URLs.
...
- Read state, saved items, installed plugin IDs, and preferences remain local. Generic feed and same-origin mirrored-image retrieval are the only normal client network requests.
