...
- The panel entry point owns a normal compositor-managed `FloatingWindow`, not a layer-shell overlay. It is resizable and maximizable, preserves the shell `open()`/`close()` lifecycle, and dismisses after another window takes focus so its floating frame never covers the app the user chose.
...
- Remote feed content, marketplace metadata, release notes, repository text, and community submissions are untrusted data. Render textual fields as bounded plain text and never execute them. Images are optional HTTPS `image.sourceUrl` values restricted to the exact marketplace preview path on `plugins.omarchy.org` or YouTube thumbnail shape on `i.ytimg.com`; marketplace PNG/JPEG/WebP bounds are validated at build time without hosting rasters, and clients must never render remote HTML, Markdown, SVG, or arbitrary image URLs.
...
- Read state, saved items, installed plugin IDs, and preferences remain local. Generic retrieval of the fixed `events.json` and optional `insights.json` companion, plus images from the two exact allowlisted marketplace/YouTube HTTPS families, are the only normal client network requests. The companion never contains private setup or reading state.
...
- Closing the window through Radar, the compositor, or shell lifecycle stops its processes. A visible bar indicator may perform one due-checked refresh at startup and at most every five minutes; hiding the indicator stops that network cadence. No daemon is installed.
...
`make local-latest` is an explicit owner-run desktop mutation, never an agent-run host test. It may install or fast-forward only the Git-managed local plugin whose origin is this exact clean checkout, collect the allowlisted live sources into a temporary tree, and import only a fully validated digest-bound edition with inspected allowlisted image references (plus validated legacy content-addressed rasters when present). It must not pull the source checkout, repoint another origin, overwrite local changes, install the shortcut, or create a watcher/daemon. The panel-only preview placement may migrate once only when its owned shell entry is exact and unmodified.
