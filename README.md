![Omarchy News Radar 0.5.8 — a persistent, keyboard-first front page for Omarchy](assets/readme-banner.svg)

# Omarchy News Radar

[![Built for Omarchy: Plugin](https://raw.githubusercontent.com/tcballard/omarchy-badges/75975e5b5bf75e7ede3764bcd2950046f7abfe2c/badges/v1/omarchy-plugin.svg)](https://plugins.omarchy.org/plugin.html?id=io.github.mtolhuys.news-radar)
...
The current interface, captured from the inspected 0.5.3 runtime with the live public feed on 6 September 2026. Versions 0.5.4 through 0.5.8 repair data completeness, Saved durability, reading actions, focus behavior, release-aware update checks, and Omarchy 4.0.3 compatibility without changing this pictured layout. Story content and counts change with the feed.

## Install the published v0.5.8
...
Version `0.5.8` is the current release. Plugin updates now follow the strictly validated manifest release version instead of every repository commit, so Forge collector and documentation changes cannot create desktop update prompts. Reading state, filters, briefing membership, and window geometry remain unchanged. See the [release notes](docs/release-notes/0.5.8.md), [verification evidence](docs/evidence/0.5.8-local.md), and [release contract](docs/RELEASE.md). Marketplace promotion remains subject to maintainer review of the exact release commit.
...
## What is new in 0.5.8

- Plugin updates are offered only when the fetched manifest declares a strictly newer release version; server collector, documentation, and other same-version commits stay silent.
- Update completion is verified against the release version rather than a transient commit SHA, so a same-version server commit landing during an update cannot produce a false failure.
- Dirty checkouts and unavailable updater binaries block only a real newer release, not an unrelated same-version server change.
...
The README screenshot shows the unchanged interface from the inspected 0.5.3 runtime. Versions 0.5.4 through 0.5.8 change data completeness, Saved durability, reading consistency, projection work, focus behavior, release-aware update checks, and Omarchy 4.0.3 compatibility rather than the pictured layout. See the [0.5.8 release notes](docs/release-notes/0.5.8.md) and [release contract](docs/RELEASE.md).
