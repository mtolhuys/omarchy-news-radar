...
- A bundled Radar application mark and newspaper-prefixed compositor title for system-facing identity. Quickshell currently exposes the shared `org.quickshell` app ID to hosted windows, so third-party switchers that ignore the window title may still choose their generic Quickshell icon.
