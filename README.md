...
Version `0.5.13` is the current release. Radar reports a newer release with a plain notice and installs nothing from inside the panel, because the repository's default branch is not the exact commit the marketplace verified; the check fetches the remote tip into a temporary repository outside the plugin, so nothing inside the installed plugin is written. Install newer releases through the Omarchy plugin marketplace. It keeps 0.5.11's fix: dismissed Omarchy News stories stay read when the rolling edition evicts and rematerializes them, and a Core retention floor keeps recent official news in the edition. No local state migration runs. See the [release notes](docs/release-notes/0.5.13.md), [verification evidence](docs/evidence/0.5.13-local.md), [D074](docs/DECISIONS.md), and [release contract](docs/RELEASE.md). Marketplace promotion remains subject to maintainer review of the exact release commit.
...
- The updater imports the repository-wide helper protocol constant instead of declaring its own version.
- CI now asserts the Python updater and QML parser contract together, preventing this mismatch from shipping unnoticed again.
