...
Item {
  ...
  function debugState() {
    ...
    return JSON.stringify({
      ...
      preferencesFocusedControl: views.preferencesDialog.buttons().filter(function(button) { return button.activeFocus }).map(function(button) { return button.label })[0] || "",
      sectionSettingsOpen: panel.sectionSettingsOpen,
      sectionSettingsFocusedControl: views.sectionSettings.buttons().filter(function(button) { return button.activeFocus }).map(function(button) { return button.label })[0] || "",
      ...
      maximized: windowLifecycle.maximized,
      fullscreen: windowLifecycle.fullscreen,
      advertisedMaximized: views.panelWindow.maximized,
      ...
      pluginUpdateState: maintenance.pluginUpdateState,
      pluginUpdateMessage: maintenance.pluginUpdateMessage,
      pluginUpdateNoticeVisible: views.masthead.pluginUpdateNotice.visible,
      pluginUpdateCheckRunning: maintenance.updateRunning,
      ...
      insightDetailReadingWidth: views.insightDetail.readingWidth,
      insightDetailHeroWidth: views.insightDetail.heroWidth,
      insightDetailRelevanceColumns: views.insightDetail.relevanceColumns,
      insightReleaseCount: views.insightDetail.releases.length,
      insightDetailId: panel.detailItem ? String(panel.detailItem.id || "") : "",
      homeRecentAdditions: (session.homeModel.recentAdditions || []).length,
      homeRecentChanges: (session.homeModel.recentChanges || []).length,
      homeActivityDate: session.homeModel.activityDate || "",
      homeRetainedDiscoveries: session.homeModel.retainedDiscoveries === true,
      insightSelectionReason: panel.detailItem ? String(panel.detailItem.selectionReason || "") : "",
      ...
      windowOpeningTrace: windowLifecycle.traceEntries,
