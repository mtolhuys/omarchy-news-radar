...
Repository-owned `content/discoveries/*.json` supplies reviewed workflow suggestions. Records include source links and review date and never authorize fetching a submitted URL. Historical builds omit records reviewed after their publication clock, and records whose project membership is unavailable are omitted after strict validation. Selection and local relevance never imply compatibility or a security endorsement.
