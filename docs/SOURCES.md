...
Forge owns one explicit degraded fallback for a failed collector or audit. `republish-last-good` reads only the validated continuity snapshot, retains its bounded events and marketplace catalog, performs no source or image request, and publishes all source-health rows as `failed` with reason `validation-failed`. It never changes source continuity, and the next ordinary run retries every source.
...
Collection searches fixed queries (`Omarchy`, `Omarchy Linux`, `Omarchy Quattro`), sanitizes descriptions (sponsor, course, affiliate, social, and URL blocks are removed), and keeps items with the keyword in the title or in strong cleaned prose. Incidental link mentions, promo-only low-information items, and narrowly marked controversy/repeated-alarm copy are dropped; language itself is never a drop reason and critical reviews stay eligible. It loads `snippet` and `statistics` via `videos.list`, and refreshes about every two hours after a successful non-empty snapshot (an empty or missing YouTube snapshot refreshes on the next collect; D046). Successful editions replace the YouTube lane with an interleaved mix of the top eight by views, likes, and recency, then apply a per-channel cap of two and a lane cap of 24 events. Thumbnails use `https://i.ytimg.com/vi/<id>/hqdefault.jpg` only. Metrics are observed facts on those events; they never create events, change IDs, or affect Front Page/significance. CI uses checked-in fixtures and never calls the live API.
...
## Reviewed release explanations and workflows (0.5.0)

`insights.json` is optional generic enrichment, not an extension of events.json. The current producer review allowlist in `radar/insights_builder.py` contains Omarchy, News Radar, Theme Manager, Layout Presets, Shelf, nejcm's Pomodoro and Focus Modes. Only exact `https://api.github.com/repos/<reviewed-owner>/<reviewed-repository>/releases?per_page=30&page=1` requests are allowed through this adapter. Catalog URLs or workflow source links cannot add network destinations. A catalog repository change cannot inherit or fetch the previous owner's notes. Responses are capped at 2 MiB, fifteen seconds and thirty releases, with no redirects; at most four run concurrently.

Drafts and prereleases do not represent stable setup updates. Version, source URL and publication date must agree; Markdown is reduced to plain text, code blocks and embedded images/HTML are removed, and only bounded original-source bullets become change entries. This extraction is deterministic and uses no AI service. Coverage can be absent or incomplete. Failures preserve matching prior validated facts when supplied by the last successful publication; successful empty indexes clear their old coverage.

Front Page discovery uses validated `plugin-added`, `plugin-released`, and `omarchy-released` events. Source titles, summaries and event dates are preserved. Matching project metadata and release notes are optional enrichment, gated by repository and exact version identity. No workflow narrative or manual discovery file is required.
