...
The adapter accepts RSS 2.0 with `title`, `link`, `guid`, `pubDate`, optional `dc:creator`, `description`, and `content:encoded`. Item URLs must stay under `https://omarchy.org/news/YYYY/MM/<slug>`. Summaries prefer `content:encoded` then description, strip HTML, and may keep up to 8,000 characters so the inspector can show the official article. List cards derive a short cleaned teaser from that body and never replace it. Event identity uses the guid permalink path slug; `occurredAt` uses `pubDate`.
