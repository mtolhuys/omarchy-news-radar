...
4. Launching the Apps entry, pressing `Super+Alt+N`, or left-clicking the newspaper summons the same window. Repeated activation while Radar has focus never toggles it closed. Choosing another window dismisses Radar so its floating frame does not cover the selected app; invoking Radar again opens it with the retained local reading state. Omarchy's `Super+Shift+N` Editor action remains unchanged.
...
7. The normal resizable window appears in ordinary task switching while active and yields by dismissing when the user switches away. The user navigates by keyboard or pointer through Front Page, For You, Core, Plugins, YouTube, and Saved, with `Tab` and `Shift+Tab` cycling sections and Down/Enter reaching finite pagination.
...
- Front Page selects at most five persistent groups from unread eligible stories: reviewed priority notices, official changes, enabled-plugin matches, and one discovery where available. Selection uses source facts and exact local IDs; it does not infer compatibility or rewrite release notes. New feed arrivals are available through an explicit New briefing action and the existing source sections.
...
- A compositor-managed resizable/maximizable window that dismisses when another app takes focus, reopens with retained reading state, and provides smooth unclipped viewport-edge story selection, explicit arrow-key/Enter finite load-more controls, a hover-visible **Check for updates** shortcut, restrained check progress, and selected/secondary text contrast across maintained themes.
- Explicit unread state on every row (dense cards use `UNREAD`/`READ` labels; quiet headlines use an unread mark), section unread badges, a per-story toggle, an explicit filtered-section **Mark all as read** action, and durable private reading state.
- A Core/Front Page reader mode: quiet headline list, dominant article pane, collapsed Details and Keys.
