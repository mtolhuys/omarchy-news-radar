...
5. Implement a normal compositor-managed movable/resizable/maximizable `FloatingWindow`, ordinary `Alt+Tab`, focus-transfer dismissal after confirmed Radar focus, responsive one/two-column presentation, and explicit selected-state contrast without changing semantic order. Omit the unreliable minimize control.
...
4. Resolve exact enabled-plugin IDs and the coalesced indicator projection in one helper. Use `refresh-if-due` from a private last-attempt timestamp at most every five minutes and only while visible. Dynamically schedule the exact next deadline, watch state, feed, setup-news and shell-plugin configuration replacement for immediate unread/health propagation, and use the shared atomic lock to contain multi-monitor overlap. Do not add a second polling timer.
...
The released 0.5.3 removes authored production discoveries. `discovery.py` selects dated source events and retains the last usable edition. The 0.5.4 candidate adds a generic setup-news companion generated from every source-dated marketplace listing in the current 30-day window. The client merges it with the rolling feed before the Plugins, Saved, and exact local For You projections; no installed ID leaves the machine. Plugin releases and retirements have protected feed capacity, while current-transition additions reserve the slots required by publication continuity before that history is selected. Release-covered projects cannot be crowded out of insights by an addition flood. Forge can invoke the networkless `republish-last-good` route after a collector or audit failure; it leaves continuity unchanged and marks every source failed. Delivery is gated on saturated-ledger, degraded-publication and companion flood regressions, disposable keyboard/visual acceptance and owner testing before any push or deployment.
