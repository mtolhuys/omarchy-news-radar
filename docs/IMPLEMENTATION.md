...
3. Implement cached-first open, one asynchronous published-edition check with visible progress and honest new/no-change/stale/offline results, section projections, exact enabled-plugin matching, deterministic front page, safe feed-image projection, icon metrics, human-facing plugin pages, search, selection, save, source opening, Tune preferences, per-section name/fixed-source/filter options, finite load-more pagination, and state labels.
