...
## Evergreen discoveries

0.5.0 also accepts bounded workflow collections under `content/discoveries/`. A record explains a concrete use and links to original project documentation, with an accurate `reviewedAt`. It must identify documentation-based suggestions honestly and cannot claim hands-on compatibility without evidence. The initial three ideas cover workspace arrangement, a focused session and a capture handoff. Each remains independent of news significance, read state and the finite briefing.

Collection slugs are permanent public identifiers. Correct copy in place, preserve attribution and source links, and update the review date when actually reviewed. A correction does not generate a news event. Catalog availability and review dates gate publication after every record is structurally validated. Release-explanation coverage has a separate explicit producer allowlist; adding a discovery does not expand it implicitly.
