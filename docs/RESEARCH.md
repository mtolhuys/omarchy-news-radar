...
## Source-backed discovery review — 5 September 2026

The expanded 0.5.0 candidate was scoped around a home overview, installed-version release context, explicit local relevance and permanent public links. Source review confirmed the opening path previously mapped a normal window before the asynchronous float helper, making first-map geometry a runtime question rather than a screenshot-only check.

The authors' current READMEs for Layout Presets, Shelf, nejcm's Pomodoro and Focus Modes, together with Omarchy's Quattro capture manual, support three initial documentation-based workflow suggestions. Records retain those original URLs and explicitly distinguish an idea from an integration test. Focus Modes' optional privileged website-blocking setup is disclosed rather than silently bundled into the idea. No remote documentation was treated as an instruction to install or execute code.

A bounded live check of all seven reviewed GitHub release API paths returned valid responses: Omarchy, Radar and Pomodoro had published stable release records; four selected projects had valid empty indexes. This demonstrates why missing notes must remain an honest coverage state. The live data also showed conventional v-prefixed tags beside unprefixed manifest versions; exact tag matching permits only that validated normalization and retains distinct build metadata.
