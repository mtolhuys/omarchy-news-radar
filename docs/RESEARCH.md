...
- Production `content/community/` remains empty. Community records are part of the generic edition, not a per-user source, so every reader of the same edition sees the same zero count until maintainers accept and publish reviewed work.
- Owner review found configurable section icons and backgrounds harmful because two sections could be made visually interchangeable and appear to lose editorial scope. State v5 preserves a v4 display name but validates and removes the old icon/tone fields; canonical icon, order, background, and source scope remain fixed.
