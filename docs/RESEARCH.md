...
- A hosted Quickshell `FloatingWindow` exposes the process-wide Wayland app ID `org.quickshell`; the current type contract has no per-window app-ID or icon setter. The separately installed `vbrosseau.alttab` 1.0.10 switcher resolves that class's desktop entry before its title, which explains the generic Quickshell gear in the supplied screenshot. Radar therefore ships a validated application SVG and a newspaper-prefixed exact compositor title, but deliberately does not shadow the global `org.quickshell.desktop` entry because that would mislabel unrelated shell windows.
