...
Plugin-update regressions use temporary real Git repositories. A newer commit with the same strict manifest version must remain `current`, including with a dirty checkout and no updater binary; a greater version alone may become `behind`; divergent history and dirt block only that greater version; and an additional same-version commit landing between inspection and official apply must still complete successfully.
