...
- Explicit per-event read/unread overrides, migrated baseline semantics, one race-safe initial-story read per fresh non-empty panel open, no rearming on summon/refresh/reprojection, deduplicated actionable indicator counts across all persistent section projections, section unread counts, unread-only filtering, bounded just-read retention within the active view, read/unread reversal, bounded atomic filtered-section marking across unloaded pages, isolation from temporary search and nonmatching sections, retention of a decision whose story left the rolling edition and its stability across the full dismiss/evict/rematerialize cycle, deterministic cap-pressure eviction of unreachable overrides only, a Core retention floor that survives a saturated release ledger, and proof that close or refresh never bulk-marks unselected stories.
...
- Catalog growth coverage: a valid catalog larger than the former 8 MiB ceiling remains accepted below the shared 16 MiB source bound; transport and fixture ingestion use that same constant.
...
5. QMP `press meta_l-alt-n` opens the rendered Radar surface through the real global shortcut route; foreground and rapid repeated activation retain exactly one focused window, transferring focus to an ordinary app dismisses Radar without leaving a helper, and the newspaper or shortcut reopens one focused instance with retained state.
6. Cached fixture content and its validated legacy same-origin raster appear without waiting for the network, focus is visible, image-off fallback is complete, and selected story fields match the validated fixture. A fresh Home open after onboarding leaves hidden briefing stories unread; choosing a briefing card reads only its selected story while another remains unread. Automatic fresh-open reading applies only to an actually visible source-reader selection. The welcome surface, Browse current stories, Home and My setup do not automatically read a story.
7. The normal window resizes by a real edge gesture and maximizes/restores. `Alt+Tab` away dismisses it so the selected tiled app is unobscured, the shortcut reopens it, and a window-manager close follows the shell lifecycle without leaving helpers.
...
10. `j`, `k`, section keys, search, save, read/unread toggle, filtered-section **Mark all as read**, **Check for updates**, Tune, and source opening use rendered controls; the batch action includes unloaded matches but not nonmatching stories, no permanent keyboard or publication-diagnostics strip occupies the reader, the update control exposes `R` on hover, dense story rows visibly state `UNREAD` or `READ`, quiet headlines expose their unread mark, section badges expose unread counts, and a guest-only inert browser shim captures the exact validated HTTPS URL.
...
`activation-upgrade.sh` separately constructs an exact local Git history from released 0.1.3 and the current candidate. It must install the 0.1.3 managed shortcut, reproduce the released background-window close through both real QMP routes, fast-forward through Omarchy's official plugin updater, and—without opening Radar or clicking any migration control—prove that the updater's normal rescan backs up and replaces only the exact retained legacy block. It must then prove that the current candidate dismisses when another app takes focus and that both the live shortcut and newspaper reopen exactly one focused Radar window while foreground repeats keep it open. It must also prove empty Hyprland configuration errors and exact shortcut removal. The panel's visible migration control remains separately source/unit tested as the rollback-safe retry path.

`public-install.sh` covers the published 0.5.0/state-v13 journey: it proves the public GitHub URL clones the supplied exact commit, validates and enables the panel, loads the fixed Forge edition, exercises the first-use choice and Front Page without hidden reads, exposes durable per-story read state, supports documented launcher and shortcut setup/removal, and removes through plugin ID `io.github.mtolhuys.news-radar`.

`release-preview.sh` is a separate networked release-only journey. It installs the release checkout only in the disposable guest, refreshes the fixed public Forge edition, requires a validated direct allowlisted marketplace/YouTube image, applies Omarchy's Matte Black theme, records exact `1240×740` window geometry below the desktop bar, captures the frame, and removes the plugin. Crop the retained `1280×800` console image by the recorded `[20,40]` origin; deterministic acceptance remains independent of this marketing proof.
...
./bin/lab doctor
RADAR_LAB_REAL_FEED=/absolute/path/to/public-preview/events.json \
RADAR_LAB_REAL_INSIGHTS=/absolute/path/to/public-preview/insights.json \
./bin/lab plugin /absolute/path/to/omarchy-news-radar/tests/lab/opening.sh
```
...
The focused briefing journey also checks both first-use choices within the window at 200% text, visits every section through real keyboard input with its rail control visibly revealed, and opens the late arrival through the briefing controls before restoring the guest font configuration. Changing text size while a moved window is open must automatically fit the usable monitor area; tests must not manually reposition it to conceal an overflow. Check the actual article viewport and reachability of controls, then close and reopen at the same enlarged text size.
...
Plugin-update regressions use temporary real Git repositories. A newer commit with the same strict manifest version must remain `current`, including with a dirty checkout; a greater version alone becomes `behind` with `canApply: false` for clean, dirty, and divergent checkouts alike. Every subprocess the check starts is recorded: only `rev-parse`, `cat-file`, `show` and `remote` may run inside the installed plugin, the single shallow fetch must run outside it, and a byte-for-byte snapshot of every file in the installed plugin, including `.git`, must be identical before and after, with no `FETCH_HEAD` created. Option-shaped, `ext::`, `fd::` and newline-bearing origin addresses planted directly in `.git/config` must be refused before any fetch. The helper must expose no install entry point, the CLI must reject `update-apply`, and the QML contract fails if any panel source reintroduces an install command, action, or button (D074).
