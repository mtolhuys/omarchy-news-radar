...
8. `Tab` and `Shift+Tab` cycle the five sections; the Settings cogwheel shows fixed sources and only actionable filters, exposes no renaming/profile path or low-value explanatory filler, and independently resets filters. In a dense list, crossing the viewport bottom anchors the complete selected row at the top, the following Down continues without moving the viewport or clipping the row, and held Up never lets repeated selection escape above the viewport; retained geometry proves all three facts. Down from the final loaded story focuses Load more with an explicit Enter label; Enter expands it without another feed request before the next Down continues into the new page.
