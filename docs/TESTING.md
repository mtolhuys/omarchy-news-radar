...
Plugin-update regressions use temporary real Git repositories. A newer commit with the same strict manifest version must remain `current`, including with a dirty checkout; a greater version alone becomes `behind` with `canApply: false` for clean, dirty, and divergent checkouts alike. Every subprocess the check starts is recorded and must be a read-only `git` subcommand, and the installed `HEAD` and worktree must be unchanged afterwards. The helper must expose no install entry point, the CLI must reject `update-apply`, and the QML contract fails if any panel source reintroduces an install command, action, or button (D074).
