...
The public plugin update check treats Git history as transport and the manifest version as the release boundary. It reads only `manifest.json` at two explicit commits, rejects objects above 64 KiB, invalid JSON, another plugin ID, and anything outside strict `major.minor.patch`. Same-version commits never become an apply request. A strictly newer version still fails closed on a dirty or divergent checkout, and execution remains delegated to Omarchy's fixed official updater command without interpolated repository data.
