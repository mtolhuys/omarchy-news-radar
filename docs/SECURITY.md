...
The public plugin update check treats Git history as transport and the manifest version as the release boundary. It reads only `manifest.json` at two explicit commits, rejects objects above 64 KiB, invalid JSON, another plugin ID, and anything outside strict `major.minor.patch`. Same-version commits stay silent, and a strictly newer version only produces a notice (D074). The check never installs and never writes inside the installed plugin. It reads the origin address from the checkout and refuses anything that is not a plain repository address, because that address becomes a git argument: a leading `-` would be read as an option and `ext::`/`fd::` transports run commands. It then fetches only the remote tip into a throwaway bare repository outside the plugin, with `--depth=1`, a 30-second limit and credential prompts disabled, and reads the fetched manifest there as data. The fetch therefore inherits none of the installed checkout's local configuration or hooks. Inside the installed plugin the check runs only `rev-parse`, `cat-file`, `show` and `remote get-url`. The marketplace verifies one exact commit; the repository's default branch is mutable, and installing it from inside the plugin would execute code outside that reviewed snapshot. A user installs a newer release through the Omarchy plugin marketplace once that release is verified there. The owner-run `make local-latest` synchronization is separate: it installs only from a clean local checkout and refuses any non-local origin, so it cannot pull the public branch either.
...
The normal optional client URLs are the fixed HTTPS companions `https://mtolhuijs.nl/news-radar/insights.json` and `https://mtolhuijs.nl/news-radar/setup-news.json`. Their independently bounded caches, HTTP validators, due metadata and locks contain generic public facts. Failure leaves the main edition readable. Installed IDs, exact manifest versions, local queries and follow/mute choices never become headers, query parameters or request bodies. Manifests are read only beneath the fixed user plugin root, must be nonsymlink regular bounded files, and must match the requested canonical ID. Unavailable or incomparable versions remain unknown.
