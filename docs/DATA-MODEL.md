...
`setup-news.json` is an optional generic companion, not a personalized feed. Its exact schema contains `schemaVersion`, `publishedAt`, at most 5,000 validated recent marketplace plugin records, and at most 500 validated plugin release/retirement events. The catalog records retain only the fields needed to reconstruct canonical `plugin-added` rows. The client merges duplicate occurrences by type, entity, and upstream occurrence time, preferring the normal feed record. It applies installed IDs, follows, mutes, filters, read state, and saves only after retrieval on the local machine.
