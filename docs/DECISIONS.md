...
**Consequence:** The prior `seenThrough` value migrates once to a fixed `readThrough` compatibility baseline. A bounded canonical boolean override map records only decisions that differ from that baseline. It originally pruned IDs outside the current edition on mutation; D073 supersedes that, because eviction from the rolling ledger is temporary. Rows, section badges, accessible names, filters, and the top-bar count consume the same predicate. Cross-process state changes are serialized so a bar preference, save, filter, or read action cannot overwrite another local mutation.
...
Live font and monitor changes also trigger a debounced fit of the exact existing floating client. A frame that still fits stays unchanged; a frame that no longer fits is resized/repositioned within the work area without taking focus or altering compositor-managed tiling/maximization. Compact reading controls scroll with the article instead of consuming its entire viewport at large text sizes. Acceptance must check the live transition and actual reading viewport, not merely capture a 200% screenshot.
...
## D066 — Reserve successor capacity for newly observed marketplace additions

**Decision:** The collector marks the exact `plugin-added` event IDs emitted by the current marketplace diff as mandatory for that successor. Retention allocates those transition-scoped rows before protected release history and ordinary priority trimming, while preserving the existing 500-event bound and canonical final order. The post-collection marketplace audit remains independent and unchanged.

**Why:** After D064 protected plugin releases, 486 retained release rows plus fourteen Core/YouTube rows filled the complete ledger. Forty-nine legitimate new listings were created by the diff and then trimmed before the audit, so the audit correctly rejected every five-minute Forge candidate and the public edition became stale. A safety gate must not be starved by the capacity policy it is checking.

**Consequence:** Any representable set of current-transition additions survives a saturated release ledger and older release history yields the required slots. Historical addition stories receive no permanent protection in the rolling ledger; setup-news remains the complete bounded marketplace companion, so the client reading universe retains them across later cycles. A missing required event, one outside the retention window, or a required set larger than the feed bound fails explicitly during successor construction; no candidate can silently advance marketplace state without its required addition stories. Source and integration tests reproduce the saturated production shape, run the independent audit, advance another collection cycle, and prove the companion still carries the addition.

## D067 — Republish validated continuity when candidate generation fails

**Decision:** Provide a separate networkless `republish-last-good` producer route for Forge to invoke only after normal collection or its independent audit fails. It rebuilds public artifacts from the validated last successful continuity snapshot, performs no source or image fetch, marks every source `failed` with the closed `validation-failed` reason at the new attempt time, and never advances source continuity. Every later scheduled run attempts the normal producer first.
...
## D069 — Make the manifest version the only client release boundary (install path superseded by D074)
...
## D073 — Keep a reading decision when its story leaves the rolling edition

**Decision:** A read/unread override is retained when its event is absent from the current edition. The override map is bounded by its existing `MAX_READ_OVERRIDES` cap rather than by edition membership: overrides for events in the current reading feed are always kept, and only when the map exceeds the cap are unreachable entries dropped, deterministically. Separately, `retain_events` reserves `CORE_RETENTION_FLOOR` slots for the newest official Core rows (20 `omarchy-news`, 3 `omarchy-released`) before the remaining protected lane competes for the 500-event bound. This supersedes the D020 clause that prunes IDs outside the current edition on mutation.

**Why:** Absence from the rolling ledger is temporary, not final, and the two facts combined destroyed user data. `diff_news` deliberately re-emits every Omarchy News item inside the rolling window on every collect so an eviction can rematerialize Core stories, and event identity is deterministic, so the same story returns under the same ID. Meanwhile the client pruned overrides to the current reading feed on every reading mutation. Plugin decisions survived eviction only because the `setup-news` companion carries `plugin-released` and `plugin-retired`; official news has no companion, so its dismissals were deleted outright. The observed public edition on 22 September 2026 carried 480 `plugin-released` rows against 5 `omarchy-news` rows, leaving an eleven-day news window, so ordinary marketplace volume evicted recent news constantly. Each cycle — dismiss, evict, prune, rematerialize — resurfaced a story the reader had already dismissed, permanently.

**Consequence:** A dismissal survives eviction and the story stays read when it returns. The override map keeps its existing 6,000-entry bound and its canonical sorted serialization, so state size, validation, and the local state schema are unchanged; no migration is required and no reading decision is altered by upgrading. Unreachable entries are only dropped under cap pressure, which cannot happen before the reading universe itself approaches the bound. The Core floor makes the news lane depth a product decision rather than a side effect of marketplace growth: recent official news survives any release volume, while the floor stays far below the 500-event bound so marketplace additions reserved by D066 and protected release history keep their capacity. Front Page composition, significance, curation, event identity, timestamps, and ordering are untouched. Source tests reproduce the saturated production shape and the full dismiss/evict/rematerialize cycle, and a mutation check proves the regression test fails against the previous behavior.

## D074 — Report newer releases, never install them from inside the plugin

**Decision:** The plugin-update check is notify-only. `inspect_update` still fetches the default-branch tip and compares strict manifest versions exactly as D069 defined, but a newer release now yields only a plain-text notice pointing at the Omarchy plugin marketplace. `apply_update`, the `update-apply` helper command, the panel's **Update plugin** and **Retry update** controls, and every call to `omarchy-plugin-update` are removed. The check reports `canApply: false` on every path. From 0.5.13 it also writes nothing inside the installed plugin: it reads the origin address from the checkout, refuses anything that is not a plain repository address (a leading `-` would be a git option, and `ext::`/`fd::` transports run commands), fetches only the remote tip (`--depth=1`, 30-second limit, no credential prompt) into a throwaway bare repository outside the plugin, and reads the fetched manifest there. Inside the installed plugin it runs only `rev-parse`, `cat-file`, `show` and `remote get-url`. This supersedes the install half of D069; its release-boundary rule stays.

**Why:** The marketplace verifies one exact commit. The repository's default branch is mutable, so running Omarchy's updater from inside the plugin installed whatever that branch named at the moment of the click — code the marketplace had not reviewed. A marketplace reviewer blocked verification of `ba3482402cf7bbc1a7ec6ebb513c58dbf61ac8eb` on exactly that: `radar/plugin_update.py` could fetch and install a newer mutable remote HEAD outside the reviewed snapshot, with the instruction to keep update checking notify-only until the target commit has separate marketplace verification. The D069 guarantees — strict version parsing, clean fast-forward requirements, delegation to the official updater — constrained *how* an install happened, but none of them tied the installed commit to a verified one.

**Consequence:** A user who opens Radar still learns that a newer release exists, and installs it through the marketplace once that release is verified. Radar no longer moves its own checkout under any condition, including clean fast-forwardable ones, and `canApply: false` means even an older panel paired with this helper cannot offer an install. Dirty and divergent checkouts are no longer special: nothing is installed, so there is nothing to block. The owner-run `make local-latest` synchronization is unaffected and was already safe: it installs only from a clean local checkout and refuses any non-local origin. The helper protocol version is unchanged (D070); removing a command and a field does not change the envelope. Source tests record every subprocess the check starts and require the installed `HEAD` and worktree to be unchanged, and a mutation check confirms that suite fails against the previous install path.
