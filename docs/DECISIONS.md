...
**Decision:** The plugin-update check is notify-only. `inspect_update` still fetches the default-branch tip and compares strict manifest versions exactly as D069 defined, but a newer release now yields only a plain-text notice pointing at the Omarchy plugin marketplace. `apply_update`, the `update-apply` helper command, the panel's **Update plugin** and **Retry update** controls, and every call to `omarchy-plugin-update` are removed. The check runs only read-only Git subcommands (`rev-parse`, `fetch`, `cat-file`, `show`) and reports `canApply: false` on every path. This supersedes the install half of D069; its release-boundary rule stays.
