...
Plugin update availability is a release-version comparison, not a repository-tip comparison. The helper fetches the default-branch tip, reads the installed and fetched `manifest.json` through bounded Git objects, requires the exact plugin ID and strict `major.minor.patch` versions, and offers an update only when the fetched version is greater. Same-version collector, documentation, publication, and server commits remain silent even for dirty installations. The check is notify-only (D074): a newer release produces a plain-text notice pointing at the Omarchy plugin marketplace, and Radar has no install action at all. The default branch is mutable, so installing whatever it names would run code outside the exact snapshot the marketplace verified; the helper therefore never runs an updater, never merges, checks out, or resets the installed checkout, fetches the remote tip only into a throwaway repository outside the plugin so nothing inside the installed plugin is written, and reports `canApply: false` on every path so an older panel paired with a newer helper cannot offer an install either.
...
news-radar-client indicator-current
...
The live feed contains a bounded rolling window. Monthly archives may retain older public events without increasing the plugin payload. Saved local items retain the fields needed to remain useful after an event leaves the live window. Projection and mutation share the same strict reconstruction boundary: a retained local save can still be read, unread, batch-read, or unsaved, while an arbitrary feed-absent ID remains inactionable.
...
While visible, one single-shot timer checks the fixed feed at most every five minutes after either a successful or failed attempt. Cadence comes from private `update-check.json`, not the edition's collection timestamp, so loading the shell shortly before an edition becomes old cannot defer the next check for another full interval. State, feed, setup-news and shell-plugin configuration watches reload the canonical unread/health indicator immediately after relevant atomic replacements; the due-check completion supplies the bounded fallback without a second polling timer. The panel does not need to be opened. Hiding the newspaper stops the timer and ignores non-state file changes; the state watch remains so Tune can restore it. This is a passive bar indicator, not a desktop notification service.
...
- Window-manager close follows the same shell hide path; maximize and resize preserve panel state, while `Alt+Tab` away dismisses Radar and preserves its local reading state for the next summon.
...
`radar/discovery.py` builds Home discovery directly from validated feed events, independent of optional insights. Under the existing state lock, it atomically retains a bounded public-only `discovery-edition.json` cache. An empty eligible feed or missing feed keeps the previous usable edition; live search, mutes, visibility and retirements still apply. Source activity dates never become refresh dates. Recent metadata is joined only after repository identity checks, and release notes require a unique exact version match.
