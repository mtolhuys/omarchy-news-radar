...
`window_rules.py` owns the exact-identity opening declaration and its token-guarded expiry. Closing disables the rule but retains its handle and complete specification; reopening with the same specification re-enables it without appending duplicate effects. Superseded one-shot timers remain armed and expire harmlessly, allowing Hyprland to release their callbacks. A late close or expiry cannot disable a newer opening. Hyprland 0.56.2 still appends effects when a named rule's specification changes and exposes no removal/replacement API; those records last until the compositor's next normal configuration reload. Radar never reloads the compositor to reclaim them. Lua callback regressions execute where the Lua runtime is available, including Plugin Lab; ordinary Python-only environments explicitly skip those three checks.
