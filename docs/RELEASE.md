...
- No publication-diagnostics strip occupies the reading surface; a subtle collapsible **Keys** footer at the bottom of the **SECTIONS** rail starts collapsed and remembers the session choice; search exposes `/` and **Check for updates** exposes `R` on hover. List cards show a short cleaned teaser; the inspector keeps the full article body.
