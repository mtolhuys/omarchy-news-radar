...
Version `0.1.4` repairs best-effort publication recovery, makes update/freshness language honest, and replaces toggle-on-activation with deterministic summon-to-focus behavior. The existing listing is maintainer-controlled; marketplace verification remains exact-commit compatibility evidence rather than a security audit.
...
6. Review the release checklist and evidence record against that exact commit. Only then create the `v0.1.4` tag and release.
