...
omarchy_host_test() {
  ...
  local helper shortcut launcher plugin_dir viewport_width viewport_height monitor_name bar_x bar_y tune_x tune_y preview_address
  ...
  wait_for_guest_state "Browse existing stories opens the reader without clearing its backlog" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.onboardingVisible == false and .storyCount > 0 and .helperRunning == false' && \
     jq -e '.onboardingComplete == true and (.readOverrides | length) == 0' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\"" || return 1
  press esc
  ...
  radar_bar_coordinates || return 1
  radar_pointer_tap "$viewport_width" "$viewport_height" "$bar_x" "$bar_y" middle
  ...
  radar_bar_coordinates || return 1
  radar_pointer_tap "$viewport_width" "$viewport_height" "$bar_x" "$bar_y" right
  ...
  radar_control_geometry tuneGeometry || return 1
  ...
  radar_pointer_tap "$viewport_width" "$viewport_height" "$tune_x" "$tune_y" left
  ...
  wait_for_guest_state "first-use failure has visible deterministic recovery" 20 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.opened == true and .status == \"No cache and failed\" and .storyCount == 0 and .noCacheNoticeVisible == true'" || {
  ...
  # new first-use choice through its rendered control before testing the
  # existing automatic first-story read on a separate fresh offline open.
  press meta_l-alt-n
  ...
  wait_for_guest_state "Browse preserves every unread decision before the fresh-open regression" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.onboardingVisible == false and .storyCount >= 2 and .helperRunning == false' && \
     jq -e '.onboardingComplete == true and (.readOverrides | length) == 0' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\"" || return 1
  press esc
  wait_for_guest_state "closing the chosen first-use view preserves the untouched backlog" 15 ssh_session \
    "hyprctl -j clients | jq -e 'all(.[]; .title != \"📰 Omarchy News Radar\")' && \
  ...
  press r
  wait_for_guest_state "Check for updates exposes its animated progress indicator while cached stories remain readable" 10 ssh_session \
  ...
  wait_for_guest_state "update-check progress clears without occupying the reading surface" 10 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.refreshing == false and .refreshIndicatorVisible == false and .status == \"No newer edition\" and (.statusDetail | startswith(\"No newer edition · published \")) and .noCacheNoticeVisible == false'" || return 1
  ssh_guest "cp /tmp/news-radar-fixtures/stale.json /tmp/news-radar-fixtures/current.json"
  press r
  wait_for_guest_state "old successful source checks never disguise publisher lag" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Publisher stale\" and .publisherStale == true and .storyCount > 0 and .noCacheNoticeVisible == false'" || return 1
  capture_console "success-news-radar-03-publisher-stale"
  ssh_guest "cp /tmp/news-radar-fixtures/recovered.json /tmp/news-radar-fixtures/current.json"
  press r
  wait_for_guest_state "fresh publication recovers from stale disclosure" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Updated\" and .publisherStale == false and (.statusDetail | startswith(\"Adopted 0 new stories · published \")) and .noCacheNoticeVisible == false'" || return 1
  ...
  capture_console "success-news-radar-03-background-shortcut-activation"
  press meta_l-alt-n
  wait_for_guest_state "foreground shortcut consistently keeps Radar open and focused" 15 ssh_session \
    "hyprctl -j activewindow | jq -e '.title == \"📰 Omarchy News Radar\"' && \
     hyprctl -j clients | jq -e '[.[] | select(.title == \"📰 Omarchy News Radar\")] | length == 1'" || return 1
  press meta_l-alt-n
  press meta_l-alt-n
  press meta_l-alt-n
  wait_for_guest_state "rapid repeated activation converges on one focused window and no helper race" 15 ssh_session \
    "hyprctl -j activewindow | jq -e '.title == \"📰 Omarchy News Radar\"' && \
     hyprctl -j clients | jq -e '[.[] | select(.title == \"📰 Omarchy News Radar\")] | length == 1' && \
     omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.opened == true and .helperRunning == false'" || return 1
  ...
  preview_address="$(ssh_session "hyprctl -j clients | jq -er '.[] | select(.title == \"📰 Omarchy News Radar\") | .address'")" || return 1
  ssh_session "hyprctl dispatch 'hl.dsp.window.resize({ window = \"address:$preview_address\", x = 1240, y = 740 })' >/dev/null && \
    hyprctl dispatch 'hl.dsp.window.move({ window = \"address:$preview_address\", x = 20, y = 40 })' >/dev/null" || return 1
  wait_for_guest_state "release preview has a complete unobscured window boundary" 10 ssh_session \
  ...
  ssh_session "hyprctl -j clients | jq '.[] | select(.address == \"$preview_address\") | {at, size, title}'" \
    >"$RUN_DIR/news-radar-preview-geometry.json" || return 1
  qmp_pointer_move "$viewport_width" "$viewport_height" 4 4 || return 1
  sleep 0.5
  capture_console "success-news-radar-preview-matte-black"
  ...
  wait_for_guest_state "Settings keeps canonical section identity and only actionable filters" 10 ssh_session \
    "jq -e '.schemaVersion == 13 and (.preferences | has(\"sectionProfiles\") | not)' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\" && \
  ...
  wait_for_guest_state "rendered filter control persists only the Plugins filter" 10 ssh_session \
    "jq -e '.schemaVersion == 13 and .preferences.sectionFilters.plugins.unreadOnly == true and .preferences.sectionFilters.core.unreadOnly == false and (.preferences.sectionFilters | has(\"community\") | not)' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\"" || return 1
  press esc
  ...
  wait_for_guest_state "retired interests are absent from UI, CLI, and current local state" 10 ssh_session \
    "! grep -q 'Apply interests\|interestField\|--interests-json' $plugin_dir/src/Panel.qml && \
     ...
     jq -e '.schemaVersion == 13 and (.preferences | has(\"interests\") | not) and (.preferences | has(\"sectionProfiles\") | not)' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\"" || return 1
  ...
  press 6
  wait_for_guest_state "numeric key six opens Saved with no Community destination" 10 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.section == \"saved\" and .storyCount == 1' && \
     ! grep -q 'Object.assign({ id: \"community\" }\|currentSection === \"community\"' $plugin_dir/src/Panel.qml" || return 1
  press o
  ...
  wait_for_guest_state "open panel adopts an externally fetched event and keeps it unread" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Updated\" and .statusDetail == \"Adopted a newer validated edition fetched in the background.\" and .section == \"for-you\" and .storyCount == $((for_you_before + 1)) and .unreadCount > 0' && \
     ...
     jq -e '.schemaVersion == 13 and .readThrough == \"1970-01-01T00:00:00Z\" and (.readOverrides | has(\"evt_000000000000000000000abc\") | not)' \"\${XDG_STATE_HOME:-\$HOME/.local/state}/omarchy-news-radar/state.json\"" || return 1
  ...
  radar_pointer_tap "$viewport_width" "$viewport_height" "$control_x" "$control_y" left
  wait_for_guest_state "the rendered read action persists only the selected event" 15 ssh_session \
  ...
  wait_for_guest_state "malformed candidate is rejected with cache intact" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Invalid feed\" and .storyCount > 0 and .noCacheNoticeVisible == false'" || return 1
  ...
  wait_for_guest_state "partial source health remains a usable edition" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Source partial\" and .storyCount > 0 and .noCacheNoticeVisible == false'" || return 1
  ...
  wait_for_guest_state "offline refresh preserves the partial last-known-good edition" 15 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.status == \"Offline\" and .storyCount > 0 and .noCacheNoticeVisible == false'" || return 1
  ...
  wait_for_guest_state "Check for updates exposes its R shortcut on hover" 10 ssh_session \
    "omarchy-shell shell call io.github.mtolhuys.news-radar debugState '' | jq -e '.refreshTooltipVisible == true'" || return 1
  sleep 0.5
  capture_console "success-news-radar-08-refresh-shortcut"
  qmp_pointer_move "$viewport_width" "$viewport_height" 4 4 || return 1
  ...
  # cannot obscure a later visual checkpoint.
  qmp_pointer_move "$viewport_width" "$viewport_height" 4 4 || return 1
  ...
  runtime_identity_before="$(sed -n 's/.*property string runtimeBuildIdentity: "\([^"]*\)".*/\1/p' "$product_root/src/Panel.qml")"
  [[ $runtime_identity_before =~ ^news-radar-[0-9]+\.[0-9]+\.[0-9]+\+identity-2$ ]] || return 1
  runtime_identity_after="${runtime_identity_before%1}2"
  ssh_session "sed -i 's|$runtime_identity_before|$runtime_identity_after|' $plugin_dir/src/Panel.qml"
  ...
  wait_for_guest_state "same-path panel update replaces the live runtime identity" 20 ssh_session \
    "test \"\$(omarchy-shell shell call io.github.mtolhuys.news-radar runtimeIdentity '')\" = '$runtime_identity_after'" || return 1
  ...
  ssh_session "test -z \"\$(hyprctl configerrors)\" && ! pgrep -u \"\$USER\" -f '([/]bin/news-radar-client|[r]adar[.]cli_client)'" || return 1
