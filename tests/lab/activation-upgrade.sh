...
omarchy_host_test() {
  ...
  wait_for_guest_state "candidate replaces plugin source" 20 ssh_session \
    "omarchy-plugin-list --json | jq -e 'any(.[]; .id == \"io.github.mtolhuys.news-radar\" and .enabled == true)' && jq -e '.version == \"0.2.1\"' $plugin_dir/manifest.json && grep -Fq 'shell summon io.github.mtolhuys.news-radar' $plugin_dir/src/BarWidget.qml" || return 1
