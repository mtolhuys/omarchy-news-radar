...
# This scenario proves the authorized public repository and fixed Forge feed.
...
omarchy_host_test() {
  ...
  wait_for_guest_state "public clone is enabled with paired panel and newspaper entry points" 20 ssh_session \
    "omarchy-plugin-list --json | jq -e 'any(.[]; .id == \"io.github.mtolhuys.news-radar\" and .enabled == true)' && \
     jq -e '.version == \"0.5.12\" and .kinds == [\"panel\",\"bar-widget\"] and (.entryPoints | keys == [\"barWidget\",\"panel\"])' $plugin_dir/manifest.json" || return 1
