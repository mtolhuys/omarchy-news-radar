...
- Restore marketplace updates after the official catalog grew beyond the old 8 MiB transport limit, using the shared source-specific 16 MiB bound sized for the declared 5,000-plugin capacity.
...
- Fix FRONT PAGE header overlap with action buttons; section labels use a reliable row layout.
- Style in-body article links with the active theme accent instead of Qt’s default blue.
- In the narrow layout, list rows stay compact until selected, then expand like an accordion.
- Refresh the YouTube lane about every two hours instead of six (still a small slice of the free API quota).
