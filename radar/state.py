...
from .io import atomic_write_json, canonical_json_bytes, ensure_private_directory, read_json_bounded, refuse_symlink
...
from .state_schema import (EPOCH, LEGACY_CLIENT_SECTIONS, LEGACY_STATE_KEYS, LEGACY_PREFERENCE_KEYS,
                           MODERN_LEGACY_STATE_KEYS, default_state, _migrate_legacy_state, _migrate_v9_state,
                           _migrate_v10_state, _migrate_v11_state, _migrate_v12_state)
    ...
    *,
    ...
    try:
        ...
        if isinstance(raw, dict) and raw.get("schemaVersion") == 11:
            ...
            atomic_write_json(path, migrated)
            return migrated, None
        if isinstance(raw, dict) and raw.get("schemaVersion") == 12:
            migrated = _migrate_v12_state(raw)
            atomic_write_json(path, migrated)
            return migrated, None
...
def save_state(state: Mapping[str, Any], environment: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    if len(canonical_json_bytes(validated)) > 512 * 1024:
        raise ValidationError("local state exceeds its byte bound")
...
def purge(environment: Mapping[str, str] | None = None) -> list[str]:
    ...
    candidates = [
        ...
        cache_root(environment) / "setup-news.json",
        cache_root(environment) / "setup-news-http.json",
        cache_root(environment) / "setup-news-check.json",
